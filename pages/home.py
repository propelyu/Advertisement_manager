...
def show_home_page():
    ...
    with ui.element("section").classes(
        ...
        with ui.column().classes(
            ...
            ui.label("THE SIMPLEST").classes("text-6xl font-bold mb-4")
            ui.label("WAY TO FIND A PROPERTY").classes("text-5xl  mb-2")
            ui.label(
                "Surrounded by nature, a peaceful retreat where modern living meets paradise"
    ...
    with ui.element("section").classes("w-full py-5"):
        with ui.element("div").classes("mx-auto max-w-7xl w-full px-6 text-center"):
            ui.label("OUR SERVICES").classes(
                "text-lg font-semibold text-pink-500 tracking-widest"
            )
            ui.label("THE SMARTEST WAY TO BUY A HOME").classes(
                "text-4xl font-bold mt-2 mb-8"
            )
        ...
        with ui.row().classes("w-full justify-center"):
            with ui.column().classes("p-4 items-center text-center max-w-sm"):
                ...
                ui.label(
                    "The smartest way to buy is now the fastest: All Cash Offer, No Downpayment, and secure Locked in Pricing.."
                ).classes("text-gray-600")
            with ui.column().classes("p-4 items-center text-center max-w-sm"):
                ui.icon("wallet").classes("text-pink-500 text-6xl mb-4")
                ui.label("All Cash Offer").classes("text-xl font-bold mb-2")
                ui.label(
                    "Stop losing homes,get the leverage of an All Cash Offer and watch sellers say YES to you."
                ).classes("text-gray-600")
            with ui.column().classes("p-4 items-center text-center max-w-sm"):
                ui.icon("lock").classes("text-pink-500 text-6xl mb-4")
                ui.label("Locked in Pricing").classes("text-xl font-bold mb-2")
                ui.label(
                    "Lock in your dream home price today. Get the certainty of Locked in Pricing and skip the financial worry."
                ).classes("text-gray-600")
    ...
    with ui.element("section").classes("w-full py-16"):
        ...
        ui.button("Load All", on_click=lambda: ui.navigate.to("/all_adverts")).props(
            "flat"
        ).classes(
            "bg-pink-400 hover:bg-yellow-700 text-white font-semibold px-5 py-2 shadow-md transition-colors flex flex-row items-center justify-center"
        )
    ...
    with ui.element("section").classes("w-full py-16 bg-white"):
        with ui.element("div").classes("mx-auto max-w-7xl w-full px-6 text-center"):
            ...
            ui.label("Happy Clients").classes(
                "text-4xl font-bold mt-2 mb-8 items-center justify-center"
            )
            ui.element("hr").classes("w-16 border-t-4 border-pink-500 mb-12 mx-auto")
        with ui.element("div").classes(
            "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-12"
        ):
            # Testimonial 1
            with ui.card().classes("p-8 bg-white shadow-lg rounded-lg"):
                ui.label(
                    "I couldn't be happier with my new property. This platform helped me find exactly what I was looking for, at a great price, and in a fantastic location."
                ).classes("text-gray-600 mb-6")
                with ui.row().classes("items-center"):
                    ui.avatar(
                        "https://cdn.pixabay.com/photo/2023/01/02/06/02/woman-7691362_1280.jpg",
                        size="md",
                    ).classes("rounded-full mr-4")
                    with ui.column():
            ...
            with ui.card().classes("p-8 bg-white shadow-lg rounded-lg"):
                ui.label(
                    "The team was incredibly helpful and professional. They were with me every step of the way, providing expert advice that made all the difference in my home search"
                ).classes("text-gray-600 mb-6")
                with ui.row().classes("items-center"):
                    ui.avatar(
                        "https://cdn.pixabay.com/photo/2021/07/03/20/06/woman-6384768_640.jpg",
                        size="md",
                    ).classes("rounded-full mr-4")
                    with ui.column():
            ...
            with ui.card().classes("p-8 bg-white shadow-lg rounded-lg"):
                ui.label(
                    "Using this website was a breeze! I found my dream home in less than a week, and the process was so simple and straightforward. Highly recommended!"
                ).classes("text-gray-600 mb-6")
                with ui.row().classes("items-center"):
                    ui.avatar(
        ...
        with ui.element("section").classes("w-full py-16"):
            with ui.element("div").classes("mx-auto max-w-7xl w-full px-6 text-center"):
                ...
                with ui.element("div").classes(
                    ...
                    with ui.card().classes(
                        "flex flex-col items-center text-center p-2 rounded-lg overflow-hidden"
                    ):
                        # Agent 1
                        with ui.element("div").classes("flex flex-col items-center"):
                            ...
                            ).classes("w-64 h-64 rounded-xl object-cover mb-4")
                    ...
                    with ui.card().classes(
                        ...
                        with ui.element("div").classes("flex flex-col items-center"):
                            ui.image(
                                "https://cdn.pixabay.com/photo/2018/06/04/22/55/books-3454396_1280.jpg"
                            ).classes("w-64 h-64 rounded-xl object-cover mb-4")
                    ...
                    with ui.card().classes(
                        ...
                        with ui.element("div").classes("flex flex-col items-center"):
                            ui.image(
                                "https://cdn.pixabay.com/photo/2024/01/07/00/36/woman-8492233_1280.jpg"
                            ).classes("w-64 h-64 rounded-xl object-cover mb-4")
                            ui.label("Hannah Normeshie").classes(
        ...
        with ui.element("div").classes("max-w-7xl mx-auto px-6"):
            with ui.element("div").classes(
                ...
                with ui.column().classes("space-y-2"):
                    ...
                    ui.link("➜ Search Properties", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                    ui.link("➜ For Agents", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                    ui.link("➜ Reviews", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                ...
                with ui.column().classes("space-y-2"):
                    ...
                    ui.link("➜ Meet the team", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                    ui.link("➜ Careers", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )

                with ui.column().classes("space-y-2"):
                    ui.label("Company").classes("text-lg font-bold text-gray-900")
                    ui.link("➜ About Us", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                    ui.link("➜ Press", "#").classes(
                        "text-pink-500 text-sm no-underline"
                    )
                ...
                with ui.column().classes("space-y-4"):
                    ui.label("Have Questions?").classes(
                    ...
                    with ui.row().classes("items-center space-x-2"):
                        ...
                        ui.link("+233 92 3929 xxx", "tel:+233923929xx").classes(
                            "text-pink-500 text-sm no-underline"
                        )
                    with ui.row().classes("items-center space-x-2"):
                        ui.icon("email").classes("text-pink-500 text-lg")
                        ui.link(
                            "info@propelproperties.com",
                            "mailto:info@propelproperties.com",
                        ).classes("text-pink-500 text-sm no-underline")

            ui.element("hr").classes("my-8 border-gray-300")
            ui.label("© 2025 Propel. All rights reserved.").classes(
                "text-center text-gray-400 text-sm w-full"
            )
